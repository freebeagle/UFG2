...
class Engine(object):
    ...
    def aggregate_clients_params(self, round_user_params):
        ...
        self.server_model_param['embedding_item.weight'].update(copy.deepcopy(updated_item_embedding))
        self._aggregate_item_text_params(round_user_params, topk_user_relation_graph, self.config['mp_layers'])

    def _aggregate_item_text_params(self, round_user_params, topk_user_relation_graph, mp_layers):
        """Channel A: item_text_proj — per-row L2 (FeDecider) + FedAvg (global semantic projector).
        Channel B: item_alpha — per-item, graph aggregation on user-user graph."""
        ...
        # Channel A: item_text_proj — L2 calibrate then FedAvg
        sum_w = None
        sum_b = None
        for params in round_user_params.values():
            w = params['item_text_proj.weight']
            w_calib = w / w.norm(dim=1, keepdim=True).clamp(min=1e-8)
            b = params['item_text_proj.bias']
            if sum_w is None:
                sum_w = w_calib
                sum_b = b
            else:
                sum_w = sum_w + w_calib
                sum_b = sum_b + b
        self.server_model_param['item_text_proj.weight'] = copy.deepcopy(sum_w / n)
        self.server_model_param['item_text_proj.bias'] = copy.deepcopy(sum_b / n)

        # Channel B: item_alpha — graph aggregation
        updated_alpha = MP_on_graph_param(
            round_user_params, 'item_alpha',
            (num_items, 1), topk_user_relation_graph, mp_layers)
        if 'item_alpha' not in self.server_model_param:
            self.server_model_param['item_alpha'] = {}
        self.server_model_param['item_alpha'].update(copy.deepcopy(updated_alpha))
    ...
    def aggregate_clients_params_user(self, round_user_params,round):
        ...
        if self.config['alias'] == 'UFGraphFR-pre':
            updated_item_embedding = MP_on_graph_with_embedding_user(round_user_params, self.config['num_items'], self.config['latent_dim'],
                                                 topk_user_relation_graph, self.config['mp_layers'])
            self.server_model_param['embedding_user.weight'].update(copy.deepcopy(updated_item_embedding))
        else:
            updated_item_embedding = MP_on_graph(round_user_params, self.config['num_items'], self.config['latent_dim'],
                                                topk_user_relation_graph, self.config['mp_layers'])
            self.server_model_param['embedding_item.weight'].update(copy.deepcopy(updated_item_embedding))
        ...
        if round_id == 0:
            ...
            if self.config['alias'] == 'UFGraphFR-pre':
                ...
                for user in range(self.config['num_users']):
                    if user not in self.server_model_param['embedding_user.weight']:
                        self.server_model_param['embedding_user.weight'][user] = copy.deepcopy(self.model.state_dict()['embedding_user.weight'].data.cpu())
                ...
                for user in tqdm.tqdm(participants, desc="Initializing Server Parameters", leave=False):
                    self.server_model_param['embedding_item.weight'][user] = copy.deepcopy(self.model.state_dict()['embedding_item.weight'].data.cpu())
                for user in range(self.config['num_users']):
                    if user not in self.server_model_param['embedding_item.weight']:
                        self.server_model_param['embedding_item.weight'][user] = copy.deepcopy(self.model.state_dict()['embedding_item.weight'].data.cpu())
                ...
                if self.config.get('use_item_attribute', False) and getattr(self.model, 'item_text_proj', None) is not None:
                    # item_text_proj: global (FedAvg), single tensor
                    ...
                    # item_alpha: per-item, per-user dict (graph aggregation)
                    self.server_model_param['item_alpha'] = {}
                    for user in range(self.config['num_users']):
                        self.server_model_param['item_alpha'][user] = copy.deepcopy(self.model.state_dict()['item_alpha'].data.cpu())
                    self.server_model_param['item_alpha']['global'] = copy.deepcopy(self.model.state_dict()['item_alpha'].data.cpu())
        ...
        for user in tqdm.tqdm(participants, desc="Training Round {}:".format(round_id + 1), leave=False):
            ...
            if round_id != 0:
                ...
                if self.config.get('use_item_attribute', False) and 'item_text_proj.weight' in self.server_model_param:
                    # item_text_proj: global tensor (FedAvg) → .data
                    # item_alpha: per-user dict (graph) → [user].data
                    if self.config['use_cuda'] is True:
                        ...
                        user_param_dict['item_alpha'] = copy.deepcopy(self.server_model_param['item_alpha'][user].data).cuda()
                    elif self.config['use_mps'] is True:
                        ...
                        user_param_dict['item_alpha'] = copy.deepcopy(self.server_model_param['item_alpha'][user].data).to('mps')
                    else:
                        ...
                        user_param_dict['item_alpha'] = copy.deepcopy(self.server_model_param['item_alpha'][user].data)
    ...
    def _copy_eval_param(self, tensor):
    ...
    def _sync_server_aggregated_params_for_eval(self, user_param_dict, user):
        """Overlay server-aggregated full2 parameters after client cache is loaded."""
        item_params = self.server_model_param.get('embedding_item.weight')
        if isinstance(item_params, dict) and user in item_params:
            user_param_dict['embedding_item.weight'] = self._copy_eval_param(item_params[user])

        if not self.config.get('use_item_attribute', False):
            return user_param_dict

        alpha_params = self.server_model_param.get('item_alpha')
        if isinstance(alpha_params, dict) and user in alpha_params:
            user_param_dict['item_alpha'] = self._copy_eval_param(alpha_params[user])

        if 'item_text_proj.weight' in self.server_model_param:
            user_param_dict['item_text_proj.weight'] = self._copy_eval_param(
                self.server_model_param['item_text_proj.weight'])
        if 'item_text_proj.bias' in self.server_model_param:
            user_param_dict['item_text_proj.bias'] = self._copy_eval_param(
                self.server_model_param['item_text_proj.bias'])
        return user_param_dict

    def fed_evaluate(self, evaluate_data,embeddingUtils, server_synced=True):
        ...
        for eval_idx in tqdm.tqdm(range(len(test_users)), desc="Evaluating...", leave=False):
            ...
            if server_synced:
                self._sync_server_aggregated_params_for_eval(user_param_dict, user)
