...
class Engine(object):
    ...
    def _aggregate_item_text_params(self, round_user_params, topk_user_relation_graph, mp_layers):
        """F3-C: keep item_text_proj L2+FedAvg; keep item_alpha local-only."""
        ...
        # Channel B intentionally absent: item_alpha stays local to each client.
        ...
        for user in tqdm.tqdm(participants, desc="Training Round {}:".format(round_id + 1), leave=False):
            ...
            if round_id != 0:
                ...
                if self.config.get('use_item_attribute', False) and 'item_text_proj.weight' in self.server_model_param:
                    ...
                    # item_alpha is local-only and is not overwritten by server state.
                    if self.config['use_cuda'] is True:
