...
class UFGraphFR(torch.nn.Module):
    def __init__(
        ...
        self.use_item_attribute = bool(config.get('use_item_attribute', False))
        self.item_text_proj = None
        self.item_attribute_alpha = float(config.get('item_attribute_alpha', 1.0))
        self.item_alpha = None
        if self.use_item_attribute:
            item_feature_path = config.get('mcp_item_feature_path')
            if item_feature_path is None or item_feature_path == '':
                item_feature_path = os.path.join(
                    'data',
                    'mcp_prepare',
                    'item_side',
                    'features',
                    'item_text_features_{}.npy'.format(config.get('item_attribute_set', 'A'))
                )
            item_text_features = np.load(item_feature_path).astype('float32')
            if item_text_features.shape[0] != self.num_items:
                raise ValueError(
                    'item_text_features rows {} != num_items {}'.format(
                        item_text_features.shape[0],
                        self.num_items
                    )
                )
            self.register_buffer('item_text_features', torch.from_numpy(item_text_features))
            self.item_text_proj = torch.nn.Linear(
                in_features=item_text_features.shape[1],
                out_features=self.latent_dim
            )
            self.item_alpha = torch.nn.Parameter(torch.full((self.num_items, 1), -2.0))  # σ(-2)≈0.12, cold items default to text
    ...
    def get_item_embedding(self, items):
        ...
        if self.use_item_attribute:
            item_text_features = self.item_text_features[items].to(torch.float32)
            item_attribute_embedding = self.item_text_proj(item_text_features)
            alpha = torch.sigmoid(self.item_alpha[items])
            item_embedding = alpha * item_embedding + (1 - alpha) * item_attribute_embedding
    ...
    def forward(self, x  : torch.Tensor, uef : torch.Tensor = None):
        ...
        item_embedding = self.get_item_embedding(items)  # 物品嵌入
