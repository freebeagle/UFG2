...
def construct_user_relation_graph_via_user(round_user_params, latent_dim, similarity_metric):
    ...
    users = sorted(round_user_params.keys())
    sample_dim = round_user_params[users[0]]['embedding_user.weight'].shape[0]
    user_embedding = np.zeros((len(users), sample_dim), dtype='float32')
    for row, user in enumerate(users):
        user_embedding[row] = copy.deepcopy(round_user_params[user]['embedding_user.weight'].numpy())
    # construct the user relation graph.
...
def MP_on_graph_param(round_user_params, param_key, param_shape, topk_user_relation_graph, layers):
    ...
    Args:
        round_user_params: dict user_id -> dict of param tensors
        param_key: key in per-user dict, e.g. 'item_text_proj.weight'
        param_shape: target reshape, e.g. (latent_dim, text_dim)
        topk_user_relation_graph: [num_users, num_users] adjacency
        layers: number of message-passing layers (>=1)
    Returns:
        dict user_id -> aggregated torch.Tensor (reshaped)
    """
    users = sorted(round_user_params.keys())
