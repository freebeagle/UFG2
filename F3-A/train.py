...
parser.add_argument('--use_item_attribute', type=str2bool, default=False)
parser.add_argument('--item_attribute_set', type=str, default='A', choices=['A', 'AB'])
parser.add_argument('--mcp_item_feature_path', type=str, default=None)
parser.add_argument('--item_attribute_alpha', type=float, default=1.0)
...
def load_mcp_split(data_dir, user_json_path=None):
    ...
    if user_json_path is None:
        user_json_path = os.path.join('data', 'mcp_prepare', 'user_side', 'raw', 'filtered_clients.json')
...
def _model_token(config):
    pre_model = str(config.get('pre_model', 'unknown'))
    if pre_model == 'HF_LOCAL':
        model_path = str(config.get('hf_embedding_model', 'local_model')).rstrip('\\/')
        return pre_model + '-' + os.path.basename(model_path)
    return pre_model
...
def train(config):
    ...
    # init logging (stdout + log file)
    ts = datetime.datetime.now().strftime('%Y%m%d_%H%M%S')
    initLogging(os.path.join('log', f'train_{ts}.log'))
    ...
    if config['dataset'] == "mcp":
        rating, train_ratings, val_ratings, test_ratings, user_infos = load_mcp_split(config['mcp_data_dir'], config.get('mcp_user_json'))
    ...
    if embeddingUtils is not None:
        detected_dim = embeddingUtils.embed_dim
        if config['embed_dim'] != detected_dim:
            print('[embed_dim auto-override] {} -> {} for pre_model={}'.format(
                config['embed_dim'], detected_dim, config['pre_model']))
