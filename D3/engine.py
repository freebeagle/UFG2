...
class Engine(object):
    ...
    def aggregate_clients_params(self, round_user_params):
        ...
        self.aggregate_item_attribute_params(
            round_user_params,
            topk_user_relation_graph,
            self.config['mp_layers'],
        )

    def aggregate_item_attribute_params(self, round_user_params, topk_user_relation_graph, mp_layers):
        """D3-clean: row-L2 calibrate item_text_proj.weight before graph aggregation."""
        ...
        text_dim = sample['item_text_proj.weight'].shape[1]
        ...
        for user, params in round_user_params.items():
            weight = params['item_text_proj.weight']
            calibrated_weight = weight / weight.norm(dim=1, keepdim=True).clamp(min=1e-8)
            calibrated_params[user] = {
                'item_text_proj.weight': calibrated_weight,
                'item_text_proj.bias': params['item_text_proj.bias'],
            }

        updated_proj_w = MP_on_graph_param(
            calibrated_params, 'item_text_proj.weight',
            (latent_dim, text_dim), topk_user_relation_graph, mp_layers)
        updated_proj_b = MP_on_graph_param(
            calibrated_params, 'item_text_proj.bias',
            (latent_dim,), topk_user_relation_graph, mp_layers)

        self.server_model_param['item_text_proj.weight'] = copy.deepcopy(updated_proj_w)
        self.server_model_param['item_text_proj.bias'] = copy.deepcopy(updated_proj_b)
    ...
    def _server_param_for_user(self, key, user):
        param = self.server_model_param[key]
        ...
        for user in tqdm.tqdm(participants, desc="Training Round {}:".format(round_id + 1), leave=False):
            ...
            if round_id != 0:
                ...
                if self.config.get('use_item_attribute', False) and 'item_text_proj.weight' in self.server_model_param:
                    proj_w = self._server_param_for_user('item_text_proj.weight', user)
                    proj_b = self._server_param_for_user('item_text_proj.bias', user)
                    alpha = self._server_param_for_user('item_alpha', user)
                    if self.config['use_cuda'] is True:
                        user_param_dict['item_text_proj.weight'] = copy.deepcopy(proj_w.data).cuda()
                        user_param_dict['item_text_proj.bias'] = copy.deepcopy(proj_b.data).cuda()
                        user_param_dict['item_alpha'] = copy.deepcopy(alpha.data).cuda()
                    elif self.config['use_mps'] is True:
                        user_param_dict['item_text_proj.weight'] = copy.deepcopy(proj_w.data).to('mps')
                        user_param_dict['item_text_proj.bias'] = copy.deepcopy(proj_b.data).to('mps')
                        user_param_dict['item_alpha'] = copy.deepcopy(alpha.data).to('mps')
                    else:
                        user_param_dict['item_text_proj.weight'] = copy.deepcopy(proj_w.data)
                        user_param_dict['item_text_proj.bias'] = copy.deepcopy(proj_b.data)
                        user_param_dict['item_alpha'] = copy.deepcopy(alpha.data)
